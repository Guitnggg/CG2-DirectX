...
ID3D12Resource* CreateBufferResource(ID3D12Device* device, size_t sizeInBytes)
{
	...
	vertexResoueceDesc.Width = sizeInBytes;
	// バッファの場合はこれらを１にする
	...
	HRESULT hr = device->CreateCommittedResource(&uploadHeepProperties, D3D12_HEAP_FLAG_NONE,
...
ID3D12DescriptorHeap* CreateDescriptorHeap(ID3D12Device* device, D3D12_DESCRIPTOR_HEAP_TYPE heapType, UINT numDescriptors, bool shadeerVisible)
{
	ID3D12DescriptorHeap* descriptorHeap = nullptr;
	D3D12_DESCRIPTOR_HEAP_DESC descriptorHeapDesc{};
	descriptorHeapDesc.Type = heapType;
	descriptorHeapDesc.NumDescriptors = numDescriptors;
	descriptorHeapDesc.Flags = shadeerVisible ? D3D12_DESCRIPTOR_HEAP_FLAG_SHADER_VISIBLE : D3D12_DESCRIPTOR_HEAP_FLAG_NONE;
...
ID3D12Resource* CreateTextrueResource(ID3D12Device* device, const DirectX::TexMetadata& metadata)
{
	// 1.metadataを基にResouceの設定
...
ID3D12Resource* CreateDepthStencilTextureResource(ID3D12Device* device, int32_t width, int32_t height)
{
	...
	heapPropaties.Type = D3D12_HEAP_TYPE_DEFAULT;  // VRAM上に作る
...
#pragma region InputLayoutの設定
	...
	inputElementDesec[1].SemanticName = "TEXCOORD";
...
#pragma region Resourceにデータを書き込む
	...
	// Sprite用のTransformationMatrix用リソースを作る。Matrix4x4　１つ分のサイズを用意する
	...
	*transformationMatrixDataSprite = MakeIdentity4x4();
	...
	// マテリアル用のリソースを作る。今回はcolor1つ分のサイズを用意する（色のサイズはVector4）
